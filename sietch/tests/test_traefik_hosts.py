...
class TestHostExtraction:
    ...
    def test_generated_proxmox_config_extracts_multiple_instances(self, tmp_path):
        """Should extract multiple Proxmox hosts from generated plain YAML."""
        proxmox_yml = tmp_path / "proxmox.yml"
        proxmox_yml.write_text(
            """# Generated by OnRamp; edit services-enabled/proxmox.external.yml instead
http:
  routers:
    proxmox:
      ...
      service: proxmox
    proxmox2:
      rule: "Host(`proxmox2.example.com`)"
      service: proxmox2
""",
            encoding="utf-8",
        )
        ...
        hosts = extractor.extract_hosts_from_file(proxmox_yml)

        assert hosts == [
            ("proxmox.example.com", "proxmox"),
            ("proxmox2.example.com", "proxmox"),
        ]
